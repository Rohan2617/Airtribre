...
function toggleMenu(category) {
    ...
    if (!isOpen) {
        ...
        tasksContainer.style.transform = 'translateY(100px)';
        const newButton = document.querySelector(`#${category} .new-task-btn`);
        newButton.style.transform = 'translateY(100px)';
    } 
    else {
        menu.classList.remove('show'); // Close menu if already open
        const tasksContainer = document.getElementById(`${category}-tasks`);
